...
const adminLayoutReducer = (state, action) => {
  switch (action.type) {
    ...
    case ACTION_TYPES.ADD_TO_HISTORY:
      const newHistory = [...state.navigationHistory, action.payload].slice(
        -10
      ); // Keep last 10
      return {
        ...state,
    ...
    case ACTION_TYPES.UPDATE_CONNECTION_STATUS:
      return {
        ...
        lastUpdate:
          action.payload === 'connected' ? new Date() : state.lastUpdate,
      };
...
export const AdminLayoutProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      dispatch({
        type: ACTION_TYPES.UPDATE_BADGE_COUNTS,
        payload: randomUpdates,
      });
        ...
        const notifications = [
          {
            ...
            message: 'Review pending vendor application',
          },
          {
            type: 'info',
            title: 'System Update',
          ...
          },
          {
            ...
            message: 'New order has been processed',
          },
        ];
  ...
  const contextValue = {
    ...
    // Computed values
    isCompactMode:
      state.isMobile || (!state.sidebarPinned && !state.sidebarOpen),
    hasUnreadNotifications: state.unreadCount > 0,
    totalBadges: Object.values(state.badgeCounts).reduce(
      (sum, count) => sum + count,
      0
    ),
  };
