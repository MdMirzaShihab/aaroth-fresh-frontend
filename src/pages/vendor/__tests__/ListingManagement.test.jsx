...
describe('ListingManagement', () => {
  ...
  beforeEach(() => {
    ...
    useBulkUpdateListingsMutation.mockReturnValue([
      mockBulkUpdateListings,
      { isLoading: false },
    ]);
    useBulkDeleteListingsMutation.mockReturnValue([
      mockBulkDeleteListings,
      { isLoading: false },
    ]);
    ...
    mockBulkUpdateListings.mockResolvedValue({
      unwrap: () => Promise.resolve({}),
    });
    mockBulkDeleteListings.mockResolvedValue({
      unwrap: () => Promise.resolve({}),
    });
  });
  ...
  describe('Page Header', () => {
    it('renders page title and description', async () => {
      ...
      expect(
        screen.getByText('Manage your product listings, inventory, and pricing')
      ).toBeInTheDocument();
    });
  ...
  describe('Search and Filters', () => {
    it('renders search bar', async () => {
      ...
      const searchInput = screen.getByPlaceholderText(
        'Search listings by product name, description...'
      );
  ...
  describe('Selection Management', () => {
    it('allows selecting individual listings', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Bulk Actions: 1 selected')
        ).toBeInTheDocument();
      });
    });
    ...
    it('allows selecting all listings', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Bulk Actions: 3 selected')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Status Toggle Integration', () => {
    it('renders ListingStatusToggle components', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId('status-toggle-listing_1')
        ).toBeInTheDocument();
        expect(
          screen.getByTestId('status-toggle-listing_2')
        ).toBeInTheDocument();
        expect(
          screen.getByTestId('status-toggle-listing_3')
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Navigation', () => {
    it('navigates to create listing page', async () => {
      ...
      const addListingButton = screen.getByRole('button', {
        name: /add listing/i,
      });
    ...
    it('navigates to edit listing page', async () => {
      ...
      await waitFor(async () => {
        ...
        expect(mockNavigate).toHaveBeenCalledWith(
          '/vendor/listings/listing_1/edit'
        );
      });
    });
  ...
  describe('Empty State', () => {
    ...
    it('shows no results message when search yields no results', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('No listings match your search criteria.')
        ).toBeInTheDocument();
      });
    });
  });
