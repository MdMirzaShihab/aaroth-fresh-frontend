        ...
        {isLoading ? (
          ...
          <>
            <div className="flex items-baseline gap-2">
              ...
              <p
                className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-text-muted'}`}
              >
                / {target}
              </p>
            </div>
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p
              className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-text-muted'} mb-2`}
            >
              {violation.description}
            </p>
            <div className="flex items-center gap-4 text-xs">
              <span
                className={`${isDarkMode ? 'text-gray-400' : 'text-text-muted'}`}
              >
...
const SLAMonitoring = ({
    ...
    () => [
      {
        ...
        color: 'sage-green',
        status:
          (metrics.uptimeSLA || 99.94) >= 99.9
            ? 'compliant'
            : (metrics.uptimeSLA || 99.94) >= 99.5
              ? 'warning'
              : 'violation',
        change: 0.02,
        trend: 'up',
      },
      {
        ...
        color: 'sage-green',
        status:
          (metrics.responseTimeSLA || 97.8) >= 95.0
            ? 'compliant'
            : (metrics.responseTimeSLA || 97.8) >= 90.0
              ? 'warning'
              : 'violation',
        change: 1.2,
        trend: 'up',
      },
      {
        ...
        color: 'earthy-yellow',
        status:
          (metrics.resolutionTimeSLA || 94.2) >= 90.0
            ? 'compliant'
            : (metrics.resolutionTimeSLA || 94.2) >= 80.0
              ? 'warning'
              : 'violation',
        change: 3.1,
        trend: 'up',
      },
      {
        ...
        color: 'dusty-cedar',
        status:
          (metrics.customerSatisfactionSLA || 96.5) >= 95.0
            ? 'compliant'
            : (metrics.customerSatisfactionSLA || 96.5) >= 90.0
              ? 'warning'
              : 'violation',
        change: 1.8,
        trend: 'up',
      },
      {
        ...
        color: 'muted-olive',
        status:
          99.8 >= 99.5 ? 'compliant' : 99.8 >= 99.0 ? 'warning' : 'violation',
        change: 0.1,
        trend: 'up',
      },
      {
        ...
        color: 'muted-olive',
        status:
          99.6 >= 99.0 ? 'compliant' : 99.6 >= 98.0 ? 'warning' : 'violation',
        change: 0.3,
        trend: 'up',
      },
    ],
      ...
      [
        {
          ...
          description:
            'API response time exceeded 500ms threshold for 15 minutes',
          severity: 'high',
        ...
        },
        {
          ...
          description:
            'User verification process exceeded 24-hour SLA commitment',
          severity: 'critical',
        ...
        },
        {
          ...
          description:
            'Database response time degraded below acceptable threshold',
          severity: 'medium',
  ...
  const complianceData = useMemo(() => {
    ...
    return (
      ...
      generateData(timeRange === '1h' ? 1 : timeRange === '24h' ? 24 : 168)
    );
  }, [data, timeRange]);
  ...
  const slaHealth = useMemo(() => {
    const compliantCount = slaTargets.filter(
    ...
    return {
      ...
      critical: slaViolations.filter(
        (v) => v.severity === 'critical' && v.status === 'active'
      ).length,
    };
  ...
  const handleChartClick = useCallback((dataPoint, context) => {
    ...
    toast.success(
      `Analyzing ${datasetLabel}: ${label} (${typeof value === 'number' ? value.toFixed(2) : value}%)`
    );
  }, []);
              ...
              >
                ...
                {slaHealth.violations > 0 && (
                  <span className="ml-2 text-tomato-red font-medium">
                    • {slaHealth.violations} active violation
                    {slaHealth.violations !== 1 ? 's' : ''}
                  </span>
                )}
              </p>
            </div>
            ...
            <div className="bg-tomato-red/10 border border-tomato-red/20 rounded-xl px-4 py-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-tomato-red">
                  {slaHealth.critical} Critical Alert
                  {slaHealth.critical !== 1 ? 's' : ''}
                </span>
              </div>
            </div>
          )}
      ...
      <Card className={`p-6 ${isDarkMode ? 'bg-gray-800/50' : 'bg-white/80'}`}>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            >
              {chartView === 'compliance' &&
                'Real-time SLA compliance monitoring across all metrics'}
              {chartView === 'violations' &&
                'SLA violation frequency and impact analysis'}
              {chartView === 'trends' &&
                'Overall SLA performance trends and patterns'}
            </p>
          </div>
