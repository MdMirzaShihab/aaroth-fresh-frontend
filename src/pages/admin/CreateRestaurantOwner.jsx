...
const CreateRestaurantOwner = () => {
  ...
  const onSubmit = async (data) => {
    try {
      // Transform address structure for backend compatibility
      const transformedData = {
        ...data,
        // Flatten address if it exists
        ...(data.address && {
          address: `${data.address.street}, ${data.address.city}, ${data.address.area}`,
      ...
      if (data.address) {
        ...
        transformedData.postalCode = data.address.postalCode;
      }

      const result = await createOwner(transformedData).unwrap();
