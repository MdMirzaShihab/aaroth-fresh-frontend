  ...
  const overallHealthScore = useMemo(() => {
    const scores = healthMetrics.map((metric) => {
      ...
      return Math.max(
        0,
        100 -
          (Math.max(0, metric.value - metric.threshold) / metric.threshold) *
            100
      );
    });
    ...
    const resourceScores = resourceMetrics.map((metric) =>
      Math.max(
        0,
        100 -
          (Math.max(0, metric.value - metric.threshold) /
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1 mb-1">
              ...
              <span
                className={`text-xs font-medium truncate ${colorScheme.text}`}
              >
                {metric.label}
              </span>
            </div>
          ...
          {metric.trend !== undefined && (
            <div className="flex-shrink-0 ml-1">
              {metric.trend > 0 ? (
                <TrendingUp
                  className={`w-3 h-3 ${metric.inverted ? 'text-tomato-red' : colorScheme.icon}`}
                />
              ) : metric.trend < 0 ? (
                <TrendingDown
                  className={`w-3 h-3 ${metric.inverted ? colorScheme.icon : 'text-tomato-red'}`}
                />
              ) : (
      ...
      >
        <div className="flex items-center justify-between mb-2 lg:mb-3">
          <div className="flex items-center gap-2">
            <IconComponent
              className={`w-3 lg:w-4 h-3 lg:h-4 ${colorScheme.icon}`}
            />
            <span
              className={`text-xs lg:text-sm font-medium ${colorScheme.text}`}
            >
              {sla.label}
            </span>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-baseline">
            <span
              className={`text-sm lg:text-lg font-bold ${colorScheme.text}`}
            >
              {sla.value.toFixed(2)}
              {sla.unit}
            </span>
  ...
  return (
    <Card className={`p-6 ${className}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {overallHealthScore >= 90 ? (
              <CheckCircle
                className={`w-6 h-6 ${isDarkMode ? 'text-sage-green' : 'text-muted-olive'}`}
              />
            ) : overallHealthScore >= 70 ? (
              <AlertTriangle
                className={`w-6 h-6 ${isDarkMode ? 'text-earthy-yellow' : 'text-earthy-brown'}`}
              />
            ) : (
      ...
      <div className="space-y-6">
        ...
        <div>
          <h4
            className={`text-sm font-semibold mb-3 ${isDarkMode ? 'text-dark-text-primary' : 'text-text-dark'}`}
          >
            Performance Metrics
          </h4>
        ...
        <div className="flex items-center justify-center pt-4 border-t border-gray-200 dark:border-gray-700">
          <div
            className={`flex items-center gap-2 text-xs ${isDarkMode ? 'text-dark-text-muted' : 'text-text-muted'}`}
          >
            <Clock className="w-3 h-3" />
            <span>Last updated: {currentTime.toLocaleTimeString()}</span>
            <div
              className={`w-1.5 h-1.5 rounded-full ${isDarkMode ? 'bg-sage-green' : 'bg-muted-olive'} animate-pulse`}
            />
          </div>
        </div>
      </div>
