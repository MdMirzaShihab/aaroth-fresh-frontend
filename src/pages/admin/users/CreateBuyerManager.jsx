/**
 * CreateBuyerManager - Buyer Manager Creation Form
 * Features: Owner assignment, permission levels, buyer selection
 */
...
import { useCreateBuyerManagerMutation } from '../../../services/admin/usersService';
          ...
          >
            ...
            <div className="max-h-40 overflow-y-auto">
              {loading ? (
                ...
                <div
                  className={`p-4 text-center text-sm ${isDarkMode ? 'text-dark-text-muted' : 'text-text-muted'}`}
                >
                  No buyer owners found
                </div>
              ) : (
...
const CreateBuyerManager = ({ isOpen, onClose, onSuccess }) => {
  const { isDarkMode } = useTheme();
  const [createManager, { isLoading }] = useCreateBuyerManagerMutation();
  ...
  // Mock buyer owners data - replace with real API call
  const mockOwners = [
    {
      id: '1',
      name: 'John Smith',
    ...
    },
    {
      id: '2',
      name: 'Maria Garcia',
      businessName: 'Garcia Family Buyer',
      email: 'maria@garciabuyer.com',
      buyerCount: 1,
    },
    {
      id: '3',
  ...
  const availablePermissions = [
    {
      id: 'view_orders',
      name: 'View Orders',
      description: 'Can view buyer orders and order history',
    },
    {
      id: 'manage_orders',
      name: 'Manage Orders',
    ...
    },
    {
      id: 'view_menu',
      name: 'View Menu',
      description: 'Can view buyer menu and items',
    },
    {
      id: 'manage_menu',
      name: 'Manage Menu',
    ...
    },
    {
      id: 'view_analytics',
      name: 'View Analytics',
      description: 'Can access buyer analytics and reports',
    },
    {
      id: 'manage_staff',
      name: 'Manage Staff',
      description: 'Can manage buyer staff and their permissions',
    },
    {
      id: 'financial_access',
  ...
  const onSubmit = useCallback(
    async (data) => {
      if (!selectedOwner) {
        toast.error('Please select a buyer owner');
        return;
      }
      ...
      try {
        ...
        toast.error('Failed to create buyer manager');
      }
    },
      ...
      <div className="p-6">
        ...
        <div className="text-center mb-6">
          ...
          <p
            className={`text-sm ${isDarkMode ? 'text-dark-text-muted' : 'text-text-muted'}`}
          >
            Add a new manager to an existing buyer
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Card className="p-6">
            <h3
              className={`text-lg font-semibold mb-4 flex items-center gap-2 ${isDarkMode ? 'text-dark-text-primary' : 'text-text-dark'}`}
            >
          ...
          <Card className="p-6">
            ...
            {selectedOwner && (
              <div
                className={`mt-4 p-4 rounded-lg ${isDarkMode ? 'bg-dark-surface' : 'bg-gray-50'}`}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    >
                      {selectedOwner.businessName} •{' '}
                      {selectedOwner.buyerCount} buyer
                      {selectedOwner.buyerCount !== 1 ? 's' : ''}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <Card className="p-6">
            ...
            <div
              className={`mt-4 p-3 rounded-lg text-sm ${isDarkMode ? 'bg-dark-surface text-dark-text-muted' : 'bg-blue-50 text-blue-800'}`}
            >
              💡 <strong>Note:</strong> Permissions can be modified later by the
              buyer owner or system administrators.
            </div>
          </Card>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={isLoading || !selectedOwner}>
              {isLoading ? (
...
export default CreateBuyerManager;
