...
import { useUpdateBuyerVerificationStatusMutation } from '../../../../store/slices/apiSlice';
import { addNotification } from '../../../../store/slices/notificationSlice';
import Button from '../../../../components/ui/Button';
import ConfirmDialog from '../../../../components/ui/ConfirmDialog';

const BuyerVerificationModal = ({ buyer, isOpen, onClose }) => {
  ...
  const [updateVerification, { isLoading }] = useUpdateBuyerVerificationStatusMutation();
    ...
    try {
      ...
      dispatch(
        addNotification({
          ...
          message: `Buyer has been ${selectedAction}${selectedAction === 'rejected' ? ' with reason provided' : ''}`,
        })
      );
  ...
  const actionConfig = {
    approved: {
      ...
      label: 'Approve Buyer',
      description: 'Grant this buyer access to the platform',
      confirmMessage: 'This buyer will be granted access to place orders and use all platform features.',
    },
    rejected: {
      icon: XCircle,
      ...
      label: 'Reject Buyer',
      description: 'Deny this buyer access to the platform',
      confirmMessage: 'This buyer will be notified of the rejection and the reason provided.',
    },
    pending: {
      icon: Clock,
      ...
      description: 'Keep this buyer in pending status for further review',
      confirmMessage: 'This buyer will remain in pending status.',
    },
  };
        ...
        <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-text-dark">Buyer Verification</h2>
            ...
            <div className="bg-gray-50 rounded-2xl p-4 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="ml-2 text-text-dark font-medium">{buyer.businessName}</span>
                </div>
                <div>
                  <span className="text-text-muted">Owner:</span>
                  <span className="ml-2 text-text-dark font-medium">{buyer.userId?.name || 'Unknown'}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="ml-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      buyer.verificationStatus === 'pending' ? 'bg-orange-50 text-orange-600' :
                      buyer.verificationStatus === 'approved' ? 'bg-sage-green/20 text-muted-olive' :
                      'bg-tomato-red/20 text-tomato-red'
                    }`}>
                      {buyer.verificationStatus?.charAt(0).toUpperCase() + buyer.verificationStatus?.slice(1)}
                    </span>
                  </span>
                </div>
              </div>
              ...
              {selectedAction === 'rejected' && (
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Provide a detailed reason for rejecting this buyer's verification..."
                    rows={4}
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <p className="text-xs text-text-muted">
                      This reason will be sent to the buyer owner via email.
                    </p>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-gray-50 p-3 rounded-xl">
                <p className="text-sm text-text-muted">
                  <strong>Buyer:</strong> {buyer.businessName}
                </p>
                <p className="text-sm text-text-muted">
                  <strong>Owner:</strong> {buyer.userId?.name || 'Unknown'}
                </p>
              </div>
            </div>
          }
...
export default BuyerVerificationModal;