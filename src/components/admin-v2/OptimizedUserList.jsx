            ...
            >
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  {...getAriaProps({
                    label: `Approve ${selectedCount} selected users`,
                  })}
                >
                ...
                <Button
                  ...
                  {...getAriaProps({
                    label: `Reject ${selectedCount} selected users`,
                  })}
                >
...
const OptimizedUserList = memo(
  ({
    ...
    const { isDarkMode } = useTheme();
    const { isMobile } = useMobileOptimization();
    const { containerRef, announce, getFocusClasses, getAriaProps } =
      useAccessibility({
        keyboard: {
          onArrowKeys: (event, key) => {
            ...
            const userRows =
              containerRef.current?.querySelectorAll('[role="row"]');
    ...
    const handleUserSelect = memoizedCallback(
      (userId, isSelected) => {
        setSelectedUsers((prev) => {
          ...
          announce(
            `${isSelected ? 'Selected' : 'Deselected'} user. ${newSelected.size} users selected`
          );
          return newSelected;
        });
      },
    ...
    const handleFilterChange = memoizedCallback(
      (key, value) => {
    ...
    const handleBulkAction = memoizedCallback(
      (action) => {
        ...
        announce(
          `Bulk ${action} action triggered for ${selectedUsers.size} users`
        );
      },
    ...
    useEffect(() => {
      const cleanup =
        componentOptimization.trackComponentMemory('OptimizedUserList');
      addCleanup(cleanup);
    }, [addCleanup]);
    ...
    if (filteredUsers.length === 0) {
      return (
        ...
        >
          <Users
            className={`w-12 h-12 mx-auto mb-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-300'}`}
          />
          <h3
            className={`text-lg font-medium mb-2 ${isDarkMode ? 'text-white' : 'text-text-dark'}`}
          >
    ...
    return (
      ...
      >
        <div id="user-list-help" className="sr-only">
          Use arrow keys to navigate between users. Press Enter or Space to
          select users.
        </div>
        ...
        >
          ...
          <div
            style={{ height: virtualState.totalHeight, position: 'relative' }}
          >
            <div
              style={{
        ...
        {import.meta.env.DEV && (
          <div className="text-xs text-gray-400 p-2 bg-gray-100 rounded">
            Virtual Scrolling: {virtualState.visibleItems.length}/
            {filteredUsers.length} items rendered | Performance:{' '}
