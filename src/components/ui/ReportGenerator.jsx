...
const ScheduleOptionCard = ({ option, isSelected, onSelect }) => {
  ...
  return (
    ...
    >
      ...
      <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-text-muted'}`}>
        {option.description}
      </p>
    </motion.div>
  );
...
const ReportGenerator = ({ 
  ...
  const reportTemplates = useMemo(() => [
    {
      ...
      icon: BarChart3,
      sections: ['overview', 'sales', 'users', 'products', 'performance', 'sla'],
      estimatedPages: 12,
    },
    {
      id: 'executive',
  ...
  const generateCSVData = useCallback(() => {
    ...
    if (includeSections.overview) {
      ...
      csvData.push(['Total Revenue', `$${analyticsData.totalRevenue || 125840}`, '+18.5%', dateRange]);
      csvData.push(['Total Orders', analyticsData.totalOrders || 3842, '+12.3%', dateRange]);
      csvData.push(['Active Users', analyticsData.activeUsers || 1567, '+24.7%', dateRange]);
      csvData.push(['Avg Order Value', `$${analyticsData.avgOrderValue || 87.65}`, '+8.9%', dateRange]);
      csvData.push([]);
    }
    ...
    if (includeSections.performance) {
      ...
      csvData.push(['API Response Time', `${performanceData.apiResponseTime || 145}ms`, '< 500ms', 'Healthy']);
      csvData.push(['System Uptime', `${performanceData.uptime || 99.94}%`, '> 99.5%', 'Compliant']);
      csvData.push(['Error Rate', `${performanceData.errorRate || 0.3}%`, '< 1%', 'Healthy']);
      csvData.push([]);
    }
  ...
  const generatePDFReport = useCallback(async () => {
    ...
    try {
      ...
      if (includeSections.overview) {
        ...
        const metrics = [
          ['Total Revenue', `$${(analyticsData.totalRevenue || 125840).toLocaleString()}`, '+18.5%'],
          ['Total Orders', (analyticsData.totalOrders || 3842).toLocaleString(), '+12.3%'],
          ['Active Users', (analyticsData.activeUsers || 1567).toLocaleString(), '+24.7%'],
          ['Avg Order Value', `$${(analyticsData.avgOrderValue || 87.65).toFixed(2)}`, '+8.9%'],
        ];
      ...
      if (includeSections.sales) {
        ...
        pdf.setFontSize(11);
        pdf.setFont('helvetica', 'normal');
        pdf.text('Revenue has shown consistent growth with strong order volume.', margin + 5, currentY);
        currentY += 6;
        pdf.text('Top performing periods indicate weekend and holiday peaks.', margin + 5, currentY);
        currentY += 15;
      }
      ...
      if (includeSections.performance) {
        ...
        const performanceMetrics = [
          ['API Response Time', `${performanceData.apiResponseTime || 145}ms`, 'Target: < 500ms'],
          ['System Uptime', `${(performanceData.uptime || 99.94).toFixed(2)}%`, 'Target: > 99.5%'],
          ['Error Rate', `${(performanceData.errorRate || 0.3).toFixed(2)}%`, 'Target: < 1%'],
          ['Active Connections', (performanceData.activeConnections || 156).toString(), 'Normal range'],
        ];
      ...
      pdf.text(`Page ${pdf.internal.pages.length - 1}`, pageWidth - margin - 20, footerY);
  ...
  }, [analyticsData, performanceData, includeSections, dateRange, selectedTemplate, onGenerateReport]);
  ...
  }, [generateCSVData, selectedTemplate, dateRange, includeSections, onGenerateReport]);
  ...
  }, [selectedTemplate, selectedSchedule, reportFormat, includeSections, recipientEmail, dateRange, onScheduleReport]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className={`p-6 ${isDarkMode ? 'bg-gray-800/50' : 'bg-white/80'}`}>
          ...
          <div className="space-y-4">
            {/* Date Range */}
            <div>
              <label className={`block text-sm font-medium mb-2 ${isDarkMode ? 'text-gray-300' : 'text-text-dark'}`}>
                Date Range
              </label>
              <select
                value={dateRange}
        ...
        >
          ...
          <div className="space-y-6">
            {includeSections.overview && (
              <div>
                <h3 className={`text-lg font-semibold mb-3 ${isDarkMode ? 'text-white' : 'text-text-dark'}`}>
                  Executive Overview
                </h3>
            ...
            <div className={`text-center py-4 text-sm ${isDarkMode ? 'text-gray-400' : 'text-text-muted'}`}>
              <p>... Additional sections will be included based on template selection ...</p>
              <p className="mt-2">
