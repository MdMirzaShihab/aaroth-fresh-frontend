...
const Toast = forwardRef(({
  ...
  const getIcon = () => {
    ...
    switch (variant) {
      case 'success':
        return <CheckCircle {...iconProps} className={cn(iconProps.className, 'text-bottle-green')} />;
      case 'error':
        return <XCircle {...iconProps} className={cn(iconProps.className, 'text-tomato-red')} />;
      case 'warning':
      ...
      case 'loading':
        return <Loader {...iconProps} className={cn(iconProps.className, 'text-text-muted animate-spin')} />;
      default:
        return <Bell {...iconProps} className={cn(iconProps.className, 'text-text-muted')} />;
    }
  };
          ...
          {action && onAction && (
            <button
              ...
              className={cn(
                ...
                variant === 'success' && 'text-bottle-green hover:text-bottle-green/80',
                variant === 'error' && 'text-tomato-red hover:text-tomato-red/80',
                variant === 'warning' && 'text-amber-700 hover:text-amber-600',
                variant === 'info' && 'text-blue-700 hover:text-blue-600',
                variant === 'default' && 'text-text-dark hover:text-text-dark/80'
              )}
            >
              {action}
            </button>
...
const containerVariants = cva('fixed z-[9999] pointer-events-none', {
  variants: {
    position: {
      ...
      'bottom-center': 'bottom-4 left-1/2 -translate-x-1/2 flex flex-col-reverse gap-3',
    },
  },
  defaultVariants: {
    position: 'top-right',
    ...
    <div className={cn(containerVariants({ position }), className)} {...props}>
      {notifications.slice(0, maxToasts).map((notification, index) => {
        ...
        if (notification.notification) {
          const { id, type, title, message, duration } = notification.notification;
          return (
...
export const SuccessToast = ({ children, ...props }) => (
  <Toast variant="success" {...props}>{children}</Toast>
);

export const ErrorToast = ({ children, ...props }) => (
  <Toast variant="error" {...props}>{children}</Toast>
);

export const WarningToast = ({ children, ...props }) => (
  <Toast variant="warning" {...props}>{children}</Toast>
);

export const InfoToast = ({ children, ...props }) => (
  <Toast variant="info" {...props}>{children}</Toast>
);

export const LoadingToast = ({ children, ...props }) => (
  <Toast variant="loading" duration={0} showProgress={false} {...props}>{children}</Toast>
);
