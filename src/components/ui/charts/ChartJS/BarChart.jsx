...
const BarChart = ({
  ...
  const chartData = useMemo(() => {
    ...
    const datasets = data.datasets ? data.datasets.map((dataset, index) => ({
      ...
      hoverBackgroundColor: dataset.hoverColor || `${dataset.color || colors[index % colors.length]}CC`,
      ...
      >
        <div className="text-center">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3 ${
            ...
            <BarElement className={`w-6 h-6 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
          </div>
