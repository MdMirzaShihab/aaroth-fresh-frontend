...
export const selectHasAdminPermission = (permission) => createSelector(
...
export const selectAdminUser = createSelector(
  [selectAuth],
  (auth) => auth.user?.role === 'admin' ? auth.user : null
);
  ...
  (api) => {
    ...
    return {
      ...
      loading: Object.values(api.queries).some(
        query => query?.endpointName === 'getAdminDashboard' && query?.status === 'pending'
      )
    };
  }
);
