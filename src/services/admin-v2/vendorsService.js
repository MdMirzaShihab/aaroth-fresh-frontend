      ...
      transformResponse: (response) => {
        ...
        return {
          ...
          totalPages:
            ...
            Math.ceil(vendors.length / (params?.limit || 20)),
...
export const transformVendorsData = (rawData) => {
  ...
  return rawData.data.map((vendor) => ({
    ...
    verificationDate: vendor.verificationDate
      ? format(new Date(vendor.verificationDate), 'PPp')
      : null,
    lastActiveAt: vendor.lastActiveAt
      ? format(new Date(vendor.lastActiveAt), 'PPp')
      : 'Never',
...
const calculateVendorRiskScore = (vendor) => {
  ...
  if (!vendor.lastActiveAt) score += 30;
    ...
    const daysSinceActive = Math.floor(
      (Date.now() - new Date(vendor.lastActiveAt)) / (1000 * 60 * 60 * 24)
    );
...
const calculateBusinessMetrics = (vendor) => {
  return {
    conversionRate:
      vendor.totalOrders > 0 && vendor.totalViews > 0
    ...
    averageOrderValue:
      vendor.totalOrders > 0
        ...
        : 0,
    listingsPerformance:
      vendor.totalListings > 0
        ? (vendor.totalOrders / vendor.totalListings).toFixed(2)
        : 0,
    customerRetention:
      vendor.returningCustomers && vendor.totalCustomers
...
export const transformVerificationData = (vendor) => {
  return {
    ...
    waitingDays: Math.floor(
      (Date.now() - new Date(vendor.createdAt)) / (1000 * 60 * 60 * 24)
    ),
    urgencyLevel: calculateVerificationUrgency(
      vendor.createdAt,
      vendor.verificationStatus
...
const calculateProfileCompleteness = (vendor) => {
  ...
  requiredFields.forEach((field) => {
    ...
    } else if (vendor[field]) completed++;
  });
