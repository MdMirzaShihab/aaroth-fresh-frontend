...
export const transformRestaurantsData = (rawData) => {
  ...
  return rawData.data.map(restaurant => ({
    ...
    lastOrderDate: restaurant.lastOrderDate ? format(new Date(restaurant.lastOrderDate), 'PPp') : 'Never',
    createdAt: format(new Date(restaurant.createdAt), 'PPp'),
    verificationDate: restaurant.verificationDate ? format(new Date(restaurant.verificationDate), 'PPp') : null,
    urgencyLevel: calculateVerificationUrgency(restaurant.createdAt, restaurant.verificationStatus),
...
const calculateRestaurantRiskScore = (restaurant) => {
  ...
  if (restaurant.lastOrderDate) {
    const daysSinceOrder = Math.floor((Date.now() - new Date(restaurant.lastOrderDate)) / (1000 * 60 * 60 * 24));
...
const calculateOrderingMetrics = (restaurant) => {
  ...
  return {
    averageOrderValue: calculateAverageOrderValue(restaurant),
    orderFrequency: restaurant.totalOrders > 0 && restaurant.createdAt 
      ? (restaurant.totalOrders / Math.max(1, Math.floor((now - new Date(restaurant.createdAt)) / (1000 * 60 * 60 * 24 * 30)))).toFixed(2)
      : 0,
...
const calculateLoyaltyScore = (restaurant) => {
  ...
  // Account age
  const monthsActive = Math.floor((Date.now() - new Date(restaurant.createdAt)) / (1000 * 60 * 60 * 24 * 30));
...
const assessComplianceStatus = (restaurant) => {
  ...
  if (!restaurant.address || !restaurant.address.street) issues.push('Missing complete address');
